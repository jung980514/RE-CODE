...
public class AuthService {
     ...
     * 처리 로직:
     * 1. 회원 존재 여부 확인
     * 2. 입력값 유효성 검사 및 trim 처리
     * 3. 전화번호 변경 시 중복 검사
     * 4. 비밀번호 변경 시 현재 비밀번호 검증 및 새 비밀번호 동일 여부 확인
     * 5. JPA Dirty Checking으로 DB에 반영
     */
    @Transactional
    public User updateUser(Long userId, UpdateUserRequest request) {
        ...
        if (phone != null && !(phone = phone.trim()).isEmpty()) {
            try {
              userRepository.findByPhone(phone)
                  ...
                  .ifPresent(other -> { throw new CustomException(ErrorCode.DUPLICATE_PHONE); });
              user.setPhone(phone);
      ...
      if (newPw != null && !(newPw = newPw.trim()).isEmpty()) {
        ...
        // 7) JPA Dirty Checking에 의해 자동 반영
        return user;
    }
