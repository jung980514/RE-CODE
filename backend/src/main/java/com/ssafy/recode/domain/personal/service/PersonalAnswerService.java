...
public class PersonalAnswerService {
    ...
    private final PersonalPromptEvaluationService personalEvaluationService;
    private final PersonalQuestionRepository personalQuestionRepository;
    private final PersonalS3UploaderService personalS3UploaderService;
    ...
    public String uploadVideoAsWav(MultipartFile file) {
        return personalS3UploaderService.uploadAsWav(file);
    }
     ...
     *  6) PersonalAnswer 엔티티에 score, isMatch, videoPath 저장
     */
    @Async
    @Transactional
    public void processAnswerAsync(Long questionId, Long userId, String wavKey) {
        try {
            ...
            // 2) 질문 조회
            PersonalQuestion question = personalQuestionRepository.findById(questionId)
                .orElseThrow(() -> new IllegalArgumentException("Invalid questionId: " + questionId));

            // 3) LLM 평가 → 0~100 점수
            double score = personalEvaluationService.evaluateAnswer(question.getContent(), sttText);

            // 4) match 여부 (예: 70점 이상이면 true)
            boolean match = score >= 80.0;
            ...
            System.out.println("✅ PersonalAnswer 저장: id=" + answer.getAnswerId()
                + ", score=" + score
                + ", match=" + match
                + ", videoPath=" + mp4Key);
        } catch (Exception e) {
